...
- **What happens when** 库存扣减成功但订单创建失败（数据库连接中断）？**System** 数据库事务自动回滚库存，补偿任务5分钟后检测并修复残留锁定
- **What happens when** 分布式锁持有者实例崩溃（持有锁时宕机）？**System** Redis锁TTL=30s到期后自动释放，其他实例可获取锁继续处理
- **What happens when** 同一用户同时提交2个订单申请同一舱位？**System** 每个订单独立获取锁，先到先得，后者返回"库存不足"
- **What happens when** 库存扣减后用户立即取消订单（15秒内）？**System** 立即释放库存并解锁，更新库存日志记录"CANCELLED"类型

#### 支付异常
- **How does system handle** 支付回调延迟或丢失？**System** 提供主动查询机制（每5分钟轮询支付状态）和定时补偿任务，确保订单状态最终一致
- **How does system handle** 用户支付金额与订单金额不符？**System** 拒绝回调并记录异常（标记SECURITY_ALERT），人工介入处理
- **How does system handle** 用户重复支付？**System** 检测重复流水号（同一订单5分钟内多笔成功支付），自动退款重复支付金额并通知用户
- **How does system handle** 支付回调到达时订单已超时取消（超过15分钟）？**System** 拒绝回调，记录日志PAYMENT_AFTER_TIMEOUT，触发人工审核是否退款
- **How does system handle** 支付网关返回成功但银行实际扣款失败（银行侧超时）？**System** 对账任务每日23:00比对支付流水，差异记录自动修复（退款或补单）
- **How does system handle** 支付回调在网络分区期间到达（服务A收到，服务B未收到）？**System** 幂等性键存储于Redis集群（跨实例共享），任意实例处理均可保证幂等
- **How does system handle** 支付签名验证通过但订单号不存在（伪造回调）？**System** 立即拒绝，记录IP地址和完整请求，触发安全告警（可能的攻击行为）

#### 库存边界情况
- **What happens when** 库存初始化时数据错误（总库存=5，已售=3，但remaining=10）？**System** 启动时执行库存校验任务，数据不一致立即告警并禁止该航次预订，人工修复后才可恢复
- **What happens when** 系统时钟跳变导致锁TTL计算错误（NTP同步导致时间回退）？**System** 锁Value包含时间戳，检测到"时间回退>1秒"时自动续期锁，防止提前释放
- **What happens when** 数据库主从延迟导致库存查询不一致（主库已扣减，从库仍显示旧值）？**System** 库存查询强制走主库（或者使用缓存+数据库双写一致性方案）
- **What happens when** 库存为负数（remaining=-3）被检测到？**System** 实时告警（Critical级别短信+电话），立即锁定该航次预订权限，启动紧急修复流程（查询日志找原因，手动调整库存）
...
- **FR-013A**: 库存变动日志格式规范：JSON格式包含{change_type: "DECREMENT/INCREMENT/LOCK/UNLOCK", cabin_inventory_id, voyage_id, cabin_type_id, quantity_change, remaining_after, order_id, user_id, operation_time(ISO8601), client_ip, trace_id}

- **FR-013B**: 库存预警机制：可用库存<阈值时触发，通知运营人员（消息中心+邮件+企业微信）；超卖检测：定时任务每5分钟扫描remaining<0的记录，立即告警并修复
...
- **FR-028A**: 库存锁定实现规范（双层锁机制）：L1层Redis分布式锁（Key: "lock:inventory:{voyage_id}:{cabin_type_id}:{order_id}", TTL=30s, 自动续期）；L2层数据库乐观锁（version字段，CAS更新）；锁等待超时=3s，最大重试=3次

- **FR-028B**: 库存扣减原子操作SQL规范：UPDATE cabin_inventory SET remaining = remaining - 1, version = version + 1, locked_count = locked_count + 1 WHERE id = ? AND version = ? AND remaining >= 1

- **FR-028C**: 分布式锁键命名规范："lock:inventory:{voyage_id}:{cabin_type_id}:{order_id}"，避免键冲突；锁释放时机：支付成功(立即释放)、超时取消(立即释放)、手动取消(立即释放)
...
- **FR-035**: 支付回调必须验证签名，支付金额必须与订单金额严格校验，防止篡改。支付处理必须实现幂等性保护

- **FR-035A**: 支付幂等性键生成规范：格式为"{order_id}:{payment_method}:{timestamp}:{random_nonce}"，使用SHA256哈希，存储于Redis，TTL=24小时

- **FR-035B**: 支付回调处理流程：① 验签 → ② 查重(幂等性键) → ③ 查订单状态 → ④ 更新库存(原子操作) → ⑤ 更新订单 → ⑥ 发送通知

- **FR-035C**: 重复支付检测机制：同一订单在5分钟内收到多个成功回调，系统自动检测并退款重复支付金额，记录异常日志
...
- **SC-003**: 库存锁定的并发操作准确率必须达到99.99%，杜绝超卖现象。具体指标：
  - 并发预订测试（1000用户同时订同一舱位）：成功率≥99.99%
  - 库存为负数事件：0次（零容忍）
  - CAS冲突率：< 1%
  - 锁等待时间P99： < 500ms
...
- **SC-004A**: 支付安全与幂等性指标（新增）：
  - 支付回调处理延迟P99： < 200ms
  - 重复支付发生率： < 0.01%
  - 签名验证失败率： < 0.001%（超过则触发安全告警）
  - 幂等性键碰撞率： < 0.0001%（24小时内）
  - 支付数据不一致事件：0次（零容忍）
...
- **SC-010A**: 代码质量与测试覆盖率指标（新增）：
  - 单元测试覆盖率：≥ 90%（核心业务逻辑≥95%）
  - 集成测试覆盖率：所有API端点100%覆盖
  - 并发测试通过率：100%（使用Go race detector无竞态条件）
  - 压力测试：支持1000并发用户，持续5分钟，成功率≥99.99%
  - 静态分析：gosec无高危漏洞，golint无ERROR级别问题
  - 代码审查：所有涉及库存和支付的代码必须通过双人审查

- **SC-011**: 数据备份必须每日执行，恢复时间目标（RTO）不超过4小时

- **SC-012**: 消息通知到达率在95%以上（微信订阅消息、短信等）

- **SC-012A**: 监控与告警覆盖率指标（新增）：
  - 核心业务指标监控覆盖率：100%（库存、支付、订单）
  - 告警响应时间：Critical告警<1分钟（库存为负、支付不一致），Warning告警<5分钟（性能下降）
  - 日志完整性：100%关键操作记录可追溯（库存变动、支付流水、订单状态变更）
  - 分布式追踪覆盖率：所有API调用链100%追踪
...
8. **时钟同步服务**: 分布式锁和日志排序依赖NTP时间同步，要求服务器间时钟偏差<100ms
...
所有库存相关实体必须包含以下字段用于并发控制：
- `version`: INT类型，乐观锁版本号，默认=0，每次更新必须+1
- `remaining`: INT类型，可用库存数量，必须>=0
- `locked_count`: INT类型，已锁定但未支付数量
- `total_count`: INT类型，总库存数量（不变）
- `last_updated_at`: TIMESTAMP，最后更新时间（用于检测死锁）

#### 1.2 库存扣减原子操作规范

必须使用数据库原子UPDATE操作，禁止先查询后更新：
...
WHERE id = ? 
  AND version = ? 
  AND remaining >= 1
  AND last_updated_at < NOW() - INTERVAL '1 minute'  -- 死锁检测
```

更新后必须检查影响行数：
- 影响行数=1：扣减成功
- 影响行数=0：扣减失败（库存不足或版本冲突）

#### 1.3 分布式锁规范

**Redis分布式锁实现**：
- **锁Key格式**: `lock:inventory:{voyage_id}:{cabin_type_id}:{order_id}`
- **锁Value**: 当前服务器ID+线程ID+时间戳
- **TTL**: 30秒（必须小于支付超时时间15分钟）
- **续期机制**: 持有锁超过20秒时自动续期（Watchdog机制）
- **释放条件**: 
  - 支付成功：立即释放
  - 支付超时：超时后释放
  - 手动取消：立即释放
...
2. 如果获取失败，重试3次（间隔100ms）
3. 如果仍然失败，返回"系统繁忙，请重试"

#### 1.4 双层锁机制

L1层（Redis分布式锁）：防止多个实例同时操作同一库存
L2层（数据库乐观锁）：防止同一实例内多线程竞争

执行顺序：
1. 获取Redis锁（超时3秒，重试3次）
2. 开启数据库事务（隔离级别：READ COMMITTED）
3. 执行CAS库存扣减SQL
4. 记录库存变动日志
5. 提交事务
6. 释放Redis锁
...
**场景1：支付成功但库存扣减失败**
- 立即回滚支付（调用退款接口）
- 记录异常日志，标记订单状态为"PAYMENT_FAILED_INVENTORY_ERROR"
- 发送短信通知用户"支付失败，款项已退回"
- 触发告警通知开发人员

**场景2：库存扣减成功但订单创建失败**
- 数据库事务自动回滚库存
- 如果事务已提交（极端情况），通过补偿任务检测并修复
- 补偿任务每5分钟检查：已扣减库存但无对应订单的记录

**场景3：分布式锁持有者崩溃**
- Redis锁TTL到期后自动释放（最多30秒延迟）
- 其他实例可以获取锁继续处理
- 崩溃前的库存操作通过数据库事务ACID保证一致性

**场景4：支付回调延迟30分钟**
- 幂等性键存储于Redis（TTL=24h），延迟到达仍可正常处理
- 如果订单已超时取消：拒绝回调，记录日志，触发人工审核
...
**幂等性键生成算法**：
```
Key = SHA256(order_id + payment_method + timestamp + random_nonce)
Format: "{order_id}:{payment_method}:{timestamp}:{hash}"
```

**存储与TTL**：
- 存储：Redis SETNX
- Key: `payment:idempotent:{order_id}`
- Value: 幂等性键
- TTL: 24小时

**处理流程**：
1. 接收回调 → 2. 验签 → 3. 查Redis（幂等性键是否存在）
   - 存在 → 返回已处理结果（不重复处理）
   - 不存在 → 4. SETNX设置键（TTL=24h）
   - 设置失败 → 其他线程正在处理，等待100ms后重试
   - 设置成功 → 5. 查询订单状态
   - 订单已支付 → 返回成功（并准备退款重复支付）
   - 订单未支付 → 6. 扣减库存 → 7. 更新订单 → 8. 记录日志
...
- 第二个及以后：
  1. 记录重复支付日志
  2. 调用退款接口原路退回
  3. 标记为"DUPLICATE_PAYMENT_REFUNDED"
  4. 发送通知给用户"检测到重复支付，款项已退回"
...
**加密要求**：
- 支付流水号：AES-256加密存储
- 用户银行卡号：仅保留后4位，其余用*代替
- 身份证号：AES-256加密，展示时仅显示前3后4位
- 手机号：展示时隐藏中间4位
...
- 库存扣减延迟：P99 < 100ms
- 锁等待时间：P99 < 500ms
- 库存为负数事件：实时告警（Critical级别）
- CAS冲突率：< 1%（超过则告警）
- 分布式锁获取失败率：< 0.1%

**告警阈值**：
- 库存<0：立即短信+电话告警
- 锁等待>1秒：Warning级别，钉钉通知
- CAS冲突率>5%：Error级别，触发代码审查
...
- 支付回调处理延迟：P99 < 200ms
- 重复支付发生率：< 0.01%
- 签名验证失败率：< 0.001%（超过则触发安全检查）
- 支付状态不一致数：必须为0（实时告警）

**告警规则**：
- 签名验证失败>10次/分钟：立即锁定支付接口，人工介入
- 重复支付>5次/小时：财务对账检查
- 支付回调延迟>5秒：检查支付网关连通性
...
所有涉及库存操作的代码必须通过以下检查：
- [ ] 使用数据库原子UPDATE，禁止SELECT+UPDATE模式
- [ ] 正确处理版本冲突（CAS失败重试3次）
- [ ] Redis锁必须设置TTL且<30秒
- [ ] 锁释放必须在finally块中（防止泄露）
- [ ] 库存变动必须记录完整日志
- [ ] 单元测试覆盖并发场景（使用-race检测）

#### 4.2 支付代码审查清单

所有支付相关代码必须通过以下检查：
- [ ] 幂等性键生成唯一性验证
- [ ] 验签顺序正确（时间戳→nonce→签名→金额）
- [ ] 敏感数据加密存储（非明文）
- [ ] 支付日志不包含完整卡号
- [ ] 退款操作有双人复核机制
- [ ] 支付回调处理有超时保护
...
**并发测试**：
- 使用Go race detector检测竞态条件：`go test -race`
- 压力测试：1000并发用户同时预订同一舱位，要求99.99%成功率
- 混沌测试：模拟Redis宕机、数据库延迟，验证系统行为
